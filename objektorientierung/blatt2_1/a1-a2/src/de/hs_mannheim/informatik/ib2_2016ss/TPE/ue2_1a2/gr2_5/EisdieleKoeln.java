...
public class EisdieleKoeln extends Eisdiele {
	
	public EisdieleKoeln(Eis eis) {
		super(eis);
	}
	...
	public Eis erstelleEis(String typ) {
		for (Eis.Eissorten c : Eis.Eissorten.values()) {
	        if (c.name().equals(typ)) {
	        	...
	        	return new EisKoeln(typ, art, behaeltnis, preis, sorten, extras);
	        }
	    }
		return null;
	}
	...
	public void begruessen() {
		System.out.println("(KÖLN) Guten Tag! Willkommen bei unserer Eisdiele.");
	}
	...
	public void kassieren(double preis) {
		System.out.println("(KÖLN) Das wären dann " + preis + " Euro");
	}
	...
	public void verabschieden(String name) {
		System.out.println("(KÖLN) Hier ist Ihr " + name + ". Guten Appetit!");
	}
	...
	public void entschuldigen(String typ) {
		System.out.println("(KÖLN) Entschuldige, aber wir haben kein " + typ);
	}
}
